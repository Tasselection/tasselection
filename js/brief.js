/**
 * BRIEF CLIENT - Application JavaScript
...
(function() {
    ...
    const SECTIONS = [
        { id: 1, name: 'Informations Générales', icon: '👁️' },
        { id: 2, name: 'Contact Client', icon: '🤝' },
        { id: 3, name: 'Informations Projet', icon: 'ℹ️' },
        { id: 4, name: 'Cible & Participants', icon: '🎯' },
        { id: 5, name: 'Contexte', icon: '❓' },
        { id: 6, name: 'Programme Prévisionnel', icon: '📅' },
        { id: 7, name: 'Concept & Expérience', icon: '💡' },
        { id: 8, name: 'Format & Contenu', icon: '🎬' },
        { id: 9, name: 'Logistique & Technique', icon: '🔗' },
        { id: 10, name: 'Identité Visuelle', icon: '👁️‍🗨️' },
        { id: 11, name: 'Budget', icon: '👛' },
        { id: 12, name: 'Contraintes & Opportunités', icon: '🔍' }
    ];
    ...
    function setupEventListeners() {
        ...
        document.querySelectorAll('.btn-add-field').forEach(btn => {
            btn.addEventListener('click', () => addRepeatableField(btn));
        });
    ...
    function updateUI() {
        ...
        const indicator = elements.stepIndicator();
        if (indicator) {
            indicator.innerHTML = `Étape <strong>${currentStep}</strong> sur <strong>${TOTAL_STEPS}</strong>`;
        }
    ...
    function addTimelineItem(btn, data = null) {
        ...
        if (!newItem.querySelector('.timeline-remove')) {
            ...
            newItem.querySelector('.timeline-remove').addEventListener('click', () => {
                newItem.remove();
                collectFormData();
                saveDraft();
            });
    ...
    function addRepeatableField(btn) {
        const container = btn?.closest('.repeatable-container');
        if (!container) return;

        const template = container.querySelector('.repeatable-item');
        if (!template) return;
        ...
        const existingRemove = newItem.querySelector('.timeline-remove');
        if (existingRemove) {
            existingRemove.addEventListener('click', () => {
                newItem.remove();
                collectFormData();
                saveDraft();
            });
        ...
        SECTIONS.forEach(section => {
            html += `
                <div class="review-section">
                    <h3 class="review-section-title">${section.icon} ${section.name}</h3>
        ...
        let html = `
            <div style="font-family: 'DM Sans', Arial, sans-serif; color: #1A1A1A; background: #FFFFFF; padding: 20px; max-width: 800px; margin: 0 auto;">
                <div style="text-align: center; margin-bottom: 30px; padding-bottom: 20px; border-bottom: 3px solid #FF6B35;">
                    <h1 style="font-family: 'Bebas Neue', Arial, sans-serif; font-size: 36px; color: #FF6B35; letter-spacing: 3px; margin: 0;">BRIEF CLIENT</h1>
        ...
        SECTIONS.forEach(section => {
            ...
            html += `
                <div style="margin-bottom: 25px; page-break-inside: avoid;">
                    <h2 style="font-family: 'Bebas Neue', Arial, sans-serif; font-size: 18px; color: #FF6B35; letter-spacing: 2px; margin-bottom: 15px; padding: 10px; background: #f8f8f8; border-left: 4px solid #FF6B35;">
